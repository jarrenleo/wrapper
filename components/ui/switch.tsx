  ...
  return (
    ...
    >
      <SwitchPrimitives.Thumb
        className={cn(
          "pointer-events-none flex h-5 w-5 items-center justify-center rounded-full bg-background shadow-md shadow-foreground/5 ring-0 transition-transform",
...
function SwitchNative({
  ...
  return (
    <Animated.View
      ...
      className={cn(
        "h-10 w-[60px] rounded-full",
        props.disabled && "opacity-50",
      )}
    >
      <SwitchPrimitives.Root
        className={cn(
          "h-10 w-[60px] shrink-0 flex-row items-center rounded-full border-2 border-transparent",
          props.checked ? "bg-primary" : "bg-input",
          className,
        )}
        {...props}
      >
        <Animated.View style={animatedThumbStyle}>
          <SwitchPrimitives.Thumb
            className={
              "flex h-9 w-9 items-center justify-center rounded-full bg-background shadow-md shadow-foreground/25 ring-0"
            }
          >
            {props.checked ? (
              <Moon size={20} className="stroke-foreground" />
            ) : (
              <Sun size={20} />
            )}
          </SwitchPrimitives.Thumb>
